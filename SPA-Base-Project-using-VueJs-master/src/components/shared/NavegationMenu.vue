...
<el-menu default-active="0">
    <template v-for="(item, i) in items">
        ...
        <el-submenu :index="i.toString()" v-if="item.children !== undefined"  v-bind:key="item.id">
            <template slot="title">
        ...
        <el-menu-item index="2" v-if="item.children === undefined" @click="redirect(item.path)" :key="item.id">
            <i :class="'fa fa-' + item.icon"></i>
            <span>{{ item.text }}</span>
        </el-menu-item>
